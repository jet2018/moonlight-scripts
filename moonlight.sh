...
case $COMMAND in
    "new")
        ...
        echo "🚀 Creating '$APP_NAME' (Tag: $TARGET_TAG)..."
        ...
        rm -rf .git && git init && git add . && git commit -m "Initial commit (v$VERSION)"
    ...
    "update")
        ...
        # Bypass GitHub Cache
        CACHE_BUSTER=$(date +%s)
        REMOTE_VERSION=$(curl -fsSL "${RAW_SCRIPT_URL}?nocache=${CACHE_BUSTER}" | grep '^VERSION=' | head -1 | cut -d '"' -f 2)

        if [ "$REMOTE_VERSION" == "$VERSION" ]; then
            echo "✅ You are already on the latest version ($VERSION)."
        else
            echo "📥 Updating: $VERSION -> $REMOTE_VERSION..."
            curl -fsSL "${RAW_SCRIPT_URL}?nocache=${CACHE_BUSTER}" -o "$0.tmp" && mv "$0.tmp" "$0" && chmod +x "$0"
    ...
    *) echo "🌕 Moonlight CLI | Usage: moonlight {new|update|version}"; exit 1 ;;
esac
